...
export default function LeadBoard({ profile, onNavigate, prefill, onPrefillConsumed }) {
  ...
  // reads newest first, so the row order follows the view you are in.
  const exportPdf = () => {
    ...
    return downloadListPdf({
      title: 'Leads',
      columns: ['Lead', 'Stage', 'Priority', 'Company', 'Location', 'Contact', 'Source', 'Owner', 'Next action'],
      rows: rows.map(l => [
        l.name,
    ...
    <div className="h-full flex flex-col">
      <div className="px-6 py-4 border-b border-bdr flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={exportPdf} className="px-3 py-1.5 text-sm text-muted border border-bdr rounded-xl transition">Export PDF</button>
