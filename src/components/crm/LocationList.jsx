...
export default function LocationList({ profile, onSelect, onNavigate }) {
  ...
  // Only the filters actually narrowing the list, so the PDF says what it is a list of.
  const activeFilters = [
  ...
  const exportPdf = () => downloadListPdf({
    title: 'Locations',
    columns: ['Location', 'Company', 'Contacts', 'City', 'Postcode', 'Status', 'Lead', 'Venue type', 'Covers'],
    ...
    footNote: 'Sorted by location name, as shown on screen.',
  });
      ...
      <div className="px-6 py-4 border-b border-bdr flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={exportPdf}
            className="px-3 py-1.5 text-sm text-muted border border-bdr rounded-xl transition">Export PDF</button>
          {canWrite && (
