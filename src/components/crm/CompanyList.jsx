...
export default function CompanyList({ profile, onSelect }) {
  ...
  const exportPdf = async () => {
    ...
    await downloadListPdf({
      title: 'Companies',
      columns: ['Company', ...(hasCountry ? ['Country'] : []), 'Lead', 'Owner', 'Locations', 'Created'],
      rows: filtered.map(c => {
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 py-3 border-b border-bdr flex items-center gap-2">
        ...
        <button onClick={exportPdf} disabled={!filtered.length}
          className="ml-auto px-3 py-1.5 text-xs text-muted border border-bdr rounded hover:text-paper transition disabled:opacity-50">
          Export PDF
        </button>
      </div>
