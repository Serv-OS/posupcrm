...
export default function QuoteBuilder({ quoteId, profile, onClose, onNavigate }) {
  ...
  // Phone builder (21): which sheet is open — 'settings' | 'add' | 'terms' | 'link' | { item: idx }
      ...
      <div className="lg:hidden flex-1 min-h-0 flex flex-col">
        <div className="px-[18px] pt-3 pb-3 border-b" style={{ borderColor: 'var(--hair)' }}>
          ...
          <button onClick={() => canWrite && setSheet('settings')} className="mt-2 w-full flex items-center gap-2.5 px-[13px] py-[11px] rounded-[11px] border text-left" style={{ background: 'var(--surface-solid)', borderColor: 'var(--ink-line)' }}>
            <span className="flex-1 min-w-0">
              <span className="block text-[13px] font-medium text-paper truncate">{quote.status}{quote.valid_until ? ` · valid until ${new Date(quote.valid_until + 'T00:00:00').toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}` : ''} · {'VAT'} per line</span>
        ...
        <div className="flex-1 overflow-y-auto p-[14px] pb-[calc(150px+env(safe-area-inset-bottom))] flex flex-col gap-3">
          <Card>
            ...
            {items.map((it, idx) => (
              <button key={idx} onClick={() => canWrite && setSheet({ item: idx })} className="w-full text-left px-[15px] py-3 border-b" style={{ borderColor: 'var(--hair)' }}>
                ...
                <div className="text-[12px] text-muted">{it.qty} × {money(it.unit_price)}{Number(it.discount) ? ` · discount ${it.discount}%` : ''}{it.category === 'saas' || it.category === 'payments' ? ` · ${CAT_LABEL[it.category]}` : ` · ${'VAT'} ${it.tax_rate ?? 20}%`}</div>
              </button>
            ))}
            {canWrite && (
          ...
          <Card>
            ...
            {[['terms', 'Terms & notes', quote.terms ? 'set' : 'none'], ['link', 'Customer link', 'copy']].map(([k, l, v], i, arr) => (
        ...
        <div className="fixed inset-x-0 z-30 px-[14px] pt-2.5 pb-2.5 border-t" style={{ bottom: 'calc(56px + env(safe-area-inset-bottom))', background: 'var(--panel-bg)', backdropFilter: 'blur(12px)', borderColor: 'var(--hair)' }}>
          <div className="flex items-baseline gap-2">
            <span className="text-[12px] text-muted flex-1">Net {money(totals.oneOff)} · {'VAT'} {money(totals.tax)}{totals.recurringArr ? ` · ARR ${money(totals.recurringArr)} ex VAT` : ''}</span>
            ...
            <EditSheet title={it.name || 'Item'} values={it} onChange={(k, v) => updateItem(idx, { [k]: v ?? '' })} onCancel={() => setSheet(null)} onSave={() => setSheet(null)} openSections={2}
              sections={[
                ...
                { title: 'Price', summary: money(lineTotal(it)), fields: [
                  { key: 'qty', label: 'Qty', type: 'number' }, { key: 'unit_price', label: `Unit ${'£'}`, type: 'number' },
                  { key: 'discount', label: 'Discount %', type: 'number' }, { key: 'tax_rate', label: `${'VAT'} %`, type: 'number' },
                ] },
              ]} />
          );
        })()}
                ...
                {items.map((it, idx) => (
                  <div key={idx} className="glass-inner rounded-xl p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                      ...
                      <div><span className="text-[9px] text-dim block">Tax %</span><input type="number" className={cell + ' w-full'} value={it.tax_rate ?? 20} onChange={e => updateItem(idx, { tax_rate: e.target.value })} /></div>
                    </div>
