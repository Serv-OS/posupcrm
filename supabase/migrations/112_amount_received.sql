-- 112: correcting the amount received on an invoice.
--
...
--   settled = amount received + credit applied TO this invoice (111)
...
-- its credit notes' credit was applied to (lowest id first, as 111 locks) and
...
-- (117_amount_received.sql) every amount in a message is in pounds.
--
-- Idempotent: safe to run twice. Apply after 109_credit_notes.sql,
-- 110_profiles_guard_role.sql and 111_credit_allocations.sql.
--
...
-- paid dates and credit note refunds it wrote are ordinary 109 and 111 columns
...
begin
  ...
  -- Whole pennies, after the 6 place float wash of the rounding rule (109).
